...
export default function ProductDetailPage({ params }: { params: any }) {
  const resolvedParams = typeof params?.then === 'function' ? use(params as Promise<any>) : params;
  const id = String((resolvedParams as any)?.id || '1');
  const { t, dir } = useI18n()
  const isRTL = dir === 'rtl'
  ...
  useEffect(() => {
    async function fetchPackageDetail() {
      try {
        const res = await apiService.public.packages.show(id);
        if (res?.data) {
          setApiProduct(res.data);
        }
      } catch (err) {
        console.warn('Package API fetch fallback notice:', err);
      }
    }
    fetchPackageDetail();
  }, [id]);
  ...
  if (apiProduct) {
    const mainCover = apiProduct.cover_url || apiProduct.cover_image || apiProduct.image || '/images/hero/01-giza.jpg';
    const backendGallery = Array.isArray(apiProduct.gallery) && apiProduct.gallery.length > 0
      ? apiProduct.gallery
      : Array.isArray(apiProduct.gallery_urls) && apiProduct.gallery_urls.length > 0
      ? apiProduct.gallery_urls
      : Array.isArray(apiProduct.images) && apiProduct.images.length > 0
      ? apiProduct.images
      : [];

    let allGalleryImages = Array.from(new Set([mainCover, ...backendGallery].filter(Boolean)));

    const demoThumbnails = [
      '/images/hero/01-giza.jpg',
      '/images/hero/02-dahab.jpg',
      '/images/hero/04-sharm.jpg',
      '/images/hero/03-hurghada.jpg',
      '/images/hero/05-luxor.jpg',
      '/images/hero/06-alexandria.jpg',
    ];

    if (allGalleryImages.length < 4) {
      for (const img of demoThumbnails) {
        if (!allGalleryImages.includes(img) && allGalleryImages.length < 5) {
    ...
    product = {
      id: String(apiProduct.id || id),
      category: apiProduct.package_type || apiProduct.category?.slug || 'tours',
      titleKey: apiProduct.name || '',
      price: apiProduct.min_price || apiProduct.price || 150,
      rating: 5,
      image: mainCover,
      images: allGalleryImages,
      location: apiProduct.destination?.name || apiProduct.location || locationParam || (isRTL ? 'مصر' : 'Egypt'),
      locationKey: '',
      duration: apiProduct.duration_label || (apiProduct.duration_days ? `${apiProduct.duration_days} Days` : 'Flexible'),
      descriptionKey: apiProduct.description || apiProduct.short_description || '',
      includes: apiProduct.includes || [
        isRTL ? 'إقامة فاخرة' : 'Premium stay',
        isRTL ? 'وجبات مشمولة' : 'Meals included',
        isRTL ? 'جولات واستكشافات' : 'Guided tours'
      ],
      itinerary: apiProduct.itinerary || [
        {
          day: 1,
          title: isRTL ? 'الوصول والاستقبال' : 'Arrival & Check-in',
          description: isRTL ? 'استقبال وتسكين في مكان الإقامة.' : 'Welcome and accommodation check-in.'
        }
      ],
    } as any;
  }
  ...
  if (!product) {
    ...
    const loc = locationParam || (isRTL ? 'مصر' : 'Egypt');
    const titleName = t(`category.${cat}`);
    const formattedTitle = isBooking && locationParam 
      ? locationParam 
      : (isRTL ? `${titleName} في ${loc}` : `${loc} - ${titleName}`);
      
    const descText = isRTL 
      ? (isBooking ? `هذه تفاصيل حجزك الخاص بـ ${formattedTitle}. نتمنى لك رحلة سعيدة!` : `استمتع بأفضل عروض ${titleName} في ${loc} المصممة خصيصاً لتجربة لا تُنسى.`)
    ...
    product = {
      ...
      location: loc,
      locationKey: '',
      duration: isRTL ? 'حسب الاختيار' : 'Custom',
      descriptionKey: descText,
      includes: [
        isRTL ? 'إقامة مميزة' : 'Premium accommodation',
        isRTL ? 'وجبات مشمولة' : 'Meals included',
        isRTL ? 'جولات سياحية' : 'Guided tours'
      ],
      itinerary: [
        {
          day: 1,
          title: isRTL ? 'الوصول والاستقبال' : 'Arrival & Welcome',
          description: isRTL ? 'استقبال في المطار أو محطة الوصول والتسكين في مكان الإقامة.' : 'Pickup and accommodation check-in.'
        },
        {
          day: 2,
          title: isRTL ? 'جولة حرة' : 'Free Tour',
          description: isRTL ? 'استكشاف أهم المعالم السياحية.' : 'Explore the main attractions.'
        }
      ]
    } as any;
  }
  ...
  const handleBooking = (e: React.FormEvent) => {
    ...
    if (!date) {
      toast.error(isRTL ? 'يرجى اختيار تاريخ السفر' : 'Please select a travel date');
      return;
    }
    if (!name || name.trim().length < 3) {
      toast.error(isRTL ? 'يرجى إدخال الاسم بالكامل (3 أحرف على الأقل)' : 'Please enter your full name (at least 3 characters)');
      return;
    }
    ...
    const totalPrice = (product?.price || 150) * (adults + children * 0.5);
        ...
        <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
          ...
          <div className="flex-1 space-y-10 w-full order-1 lg:order-2">
            <section className="space-y-4">
              ...
              <div className="flex gap-3 overflow-x-auto pb-3 pt-2 no-scrollbar scroll-smooth">
                {images.map((img: string, i: number) => (
                  <button
                    key={i}
                    type="button"
                    onClick={() => setActiveImg(i)}
                    className={`relative w-28 md:w-36 aspect-video rounded-2xl overflow-hidden shrink-0 border-2 transition-all duration-300 cursor-pointer ${
                      activeImg === i
                        ? 'border-cyan ring-4 ring-cyan/20 scale-105 z-10 shadow-lg shadow-cyan/20'
                  ...
                  <div className="max-w-none">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-8 border-y border-white/5">
                      {[
                        ...
                        { icon: MapPin, label: t('product.location'), value: t(product?.locationKey || product?.location || '') },
